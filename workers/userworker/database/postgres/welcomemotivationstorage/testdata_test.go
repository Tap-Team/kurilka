...
func userWelcomeMotivation(db *sql.DB, userId int64) (model.Motivation, error) {
	query := fmt.Sprintf(
...
func insertUser(db *sql.DB, userId int64, welcomeMotivationId int) error {
	query := fmt.Sprintf(
	...
	_, err := db.Exec(query, userId, "dima", 1, 1, 1.0, welcomeMotivationId)
	return err
}
